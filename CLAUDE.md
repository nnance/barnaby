...
| onnxruntime logs from C++, so `try/except` cannot silence it | The window probe runs sizes the graph may reject. A rejection is an ordinary Python exception we catch — but onnxruntime has already written three `ExecuteKernel ... LSTM` lines to stderr first, so every startup printed a wall of errors for a probe working as designed. Fix is `SessionOptions.log_severity_level = 4` on a **separate probe session**; the real session keeps default logging. Rejections are now one `1024:rejected` field in our own debug line |
| The VAD graph takes exactly **one analysis frame** per call | Measured on this export: 256-640 samples run, anything outside raises. So of `CANDIDATE_WINDOWS`, 1024 and 1536 can never work here — kept anyway, since the next export may differ, and they no longer cost log noise. Note `input` is declared `[None, None]`, so a candidate cannot be pre-validated against the model's dims; it has to be run |
...
| ALSA levels do not survive a reboot | **Now handled in config**: `audio.playback_volume: [100, 90]` is applied to every playback stage at startup, so a fresh Pi or one that lost its mixer state comes up right. Before that it needed `sudo alsactl store`, which `alsa-restore` only writes on a *clean* shutdown — a power cut reverted it and looked like playback broke |
